   ...
   NavHost(
      ...
      composable<Routes.Detail> { backstackEntry ->
         ...
         DetailScreen(
            petId = route.petId,
            onAdoptClick = {
               navController.navigate(Routes.Adopt(route.petId))
            },
            onUpClick = { navController.navigateUp() }
         )
      }
      composable<Routes.Adopt> { backstackEntry ->
         val route: Routes.Adopt = backstackEntry.toRoute()
         AdoptScreen(
            petId = route.petId,
            onUpClick = { navController.navigateUp() }
         )
...
fun ListScreen(
   ...
   Scaffold(
      topBar = { PetAppBar(title = "Find a Friend") }
   ) { innerPadding ->
      LazyVerticalGrid(
         ...
         items(viewModel.petList) { pet ->
            Image(
               ...
               modifier = Modifier.clickable { onImageClick(pet) }
            )
         }
      }
   }
...
fun DetailScreen(
   ...
   Scaffold(
      topBar = { PetAppBar("Details", canNavigateBack = true, onUpClick = onUpClick) }
   ) { innerPadding ->
...
fun AdoptScreen(
   ...
   Scaffold(
      topBar = { PetAppBar("Thank You!", canNavigateBack = true, onUpClick = onUpClick) }
   ) { innerPadding ->
      Column(modifier = modifier.padding(innerPadding)) {
         ...
         Text(
            "Please pick up your new family member during business hours.",
            modifier = modifier.padding(6.dp)
         )
         Button(
            onClick = { /* Share not required */ },
            modifier = modifier.padding(6.dp)
         ) {
...
fun PreviewDetailScreen() {
   ...
   PetAdoptionTheme {
      DetailScreen(petId = pet.id, onAdoptClick = {})
   }
}
